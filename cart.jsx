...
const Products = (props) => {
  ...
  const addToCart = (e) => {
    ...
    let listIndex = e.target.index;
    let newItem = items.filter((item, index) => item.name == name && item.instock > 0);
    if (newItem[0].instock == 0) return;
    newItem[0].instock = newItem[0].instock - 1;
    setCart([...cart, newItem[0]]);
  };
  ...
  const deleteCartItem = (index) => {
    ...
    let target = cart.filter((item, newi) => index == newi);
    
    let newItems = items.map((item, i3) => {
